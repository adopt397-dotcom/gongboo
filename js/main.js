// ============================================================
// T123-1 + T123-2 + T123-3 + T123-4 통합 (GitHub용 - 완전한 버전) ----- 선행 코드
// ============================================================
...
// ============================================================
// A: renderGraphic() - 통합 렌더러 (위에서 이미 제공 완료)
// ============================================================

// ============================================================
// x의 나머지 함수들 (renderGraphic 이후에 오는 함수들)
// ============================================================
...
function renderGraphic(jsonData) {
  ...
  // ============================================================
    ...
    setTimeout(function() {
      ...
      var labels = parsedData.labels || parsedData.categories || [];
      ...
      if (parsedData.series && Array.isArray(parsedData.series)) {
        datasets = parsedData.series.map(function(s, i) {
          ...
          if (typeof data === 'object' && !Array.isArray(data)) {
            data = labels.map(function(label) { return data[label] || 0; });
          }
          return {
        ...
        datasets = [{
          ...
          data: values,
          backgroundColor: (parsedData.color || '#3498db') + '80',
      ...
      var cc = {
        ...
        options: {
          ...
          scales: {
            x: { grid: { color: '#e0e0e0' } },
            y: { beginAtZero: true, title: { display: true, text: parsedData.yAxis?.label || '' }, grid: { color: '#e0e0e0' }, min: yMin, max: yMax }
          }
        }
      };
  ...
  // ============================================================
    ...
    setTimeout(function() {
      ...
      var categories = parsedData.xAxis?.categories || [];
      ...
      var cc = {
        ...
        options: {
          ...
          scales: {
            x: { title: { display: true, text: parsedData.xAxis?.title || '' }, grid: { color: '#e0e0e0' } },
            y: { beginAtZero: true, title: { display: true, text: parsedData.yAxis?.title || '' }, grid: { color: '#e0e0e0' } }
          }
        }
      };
  ...
  // 5. SCATTER
  // ============================================================
  else if (parsedData.type === 'scatter' || parsedData.type === 'scatter-only') {
    setTimeout(function() {
      var ctx = document.getElementById(chartId);
      if (!ctx) return;
      if (window._chartInstances && window._chartInstances[chartId]) {
      ...
      // 추세선
      if (parsedData.line || parsedData.segments) {
        var linePoints = parsedData.segments || parsedData.line?.points || [];
        if (linePoints.length > 0) {
          var lineData = linePoints.map(function(p) {
            return { x: p.from ? p.from[0] : p.x, y: p.from ? p.from[1] : p.y };
          });
          if (linePoints[0] && linePoints[0].to) {
            lineData = linePoints.map(function(p) {
              return { x: p.from[0], y: p.from[1] };
            });
          }
          datasets.push({
      ...
      var cc = {
        ...
        options: {
          ...
          scales: {
            x: { title: { display: true, text: parsedData.xAxis?.label || '' }, grid: { color: '#e0e0e0' }, min: xMin, max: xMax },
            y: { title: { display: true, text: parsedData.yAxis?.label || '' }, grid: { color: '#e0e0e0' }, min: yMin, max: yMax }
          }
        }
      };
